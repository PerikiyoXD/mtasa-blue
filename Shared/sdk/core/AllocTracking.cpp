...
public:
    ...
    maptype<std::uint64_t, uint> sizeSortedTagMap;
    ...
    {
        std::uint64_t key = ((std::uint64_t)size << 32) | (std::uint64_t)tagId;
...
MTAEXPORT unsigned long GetAllocStats(uint uiType, void* pOutData, unsigned long ulNumStats)
{
    ...
    else if (uiType == 1)
    {
        ...
        uint idx = 0;
        for (maptype<std::uint64_t, uint>::reverse_iterator it = pAllocStats->sizeSortedTagMap.rbegin(); it != pAllocStats->sizeSortedTagMap.rend(); ++it)
        {
            if (idx >= ulNumStats)
                break;
