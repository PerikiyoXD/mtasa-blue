...
// Get a file size
//
std::uint64_t SharedUtil::FileSize(const SString& strFilename)
{
    // Open
    FILE* fh = File::Fopen(strFilename, "rb");
    if (!fh)
    ...
    fseek(fh, 0, SEEK_END);
#ifdef _WIN32
    std::uint64_t size = _ftelli64(fh);
#elif defined(__APPLE__)
    std::uint64_t size = ftello(fh);
#else
    std::uint64_t size = ftello64(fh);
#endif
    // Close
    fclose(fh);
    return size;
...
std::vector<SString> SharedUtil::FindFiles(const SString& strInMatch, bool bFiles, bool bDirectories, bool bSortByDate)
{
    ...
    std::multimap<std::uint64_t, SString> sortMap;
    ...
    if (hFind != INVALID_HANDLE_VALUE)
    {
        do
        {
            if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? bDirectories : bFiles)
                ...
                {
                    if (bSortByDate)
                        MapInsert(sortMap, (std::uint64_t&)findData.ftLastWriteTime, ToUTF8(findData.cFileName));
                    else
    ...
    if (!sortMap.empty())
    {
        for (std::multimap<std::uint64_t, SString>::iterator iter = sortMap.begin(); iter != sortMap.end(); ++iter)
            ...
            if (strcmp(DirEntry->d_name, ".") && strcmp(DirEntry->d_name, ".."))
            {
                ...
                if (bIsDir ? bDirectories : bFiles)
                {
                    if (bSortByDate)
                    {
                        ...
                        MapInsert(sortMap, (std::uint64_t)attrib.st_mtime, SStringX(DirEntry->d_name));
                    }
                    else
